...
export default function UsersPage() {
    ...
    const fetchData = async () => {
        try {
            const [uRes, rRes] = await Promise.all([
                fetch(`${process.env.NEXT_PUBLIC_API_URL || "https://efaschoolburewala.onrender.com"}/users`),
                fetch(`${process.env.NEXT_PUBLIC_API_URL || "https://efaschoolburewala.onrender.com"}/roles`)
            ]);
    ...
    const handleDelete = async (id: number) => {
        ...
        try {
            await fetch(`${process.env.NEXT_PUBLIC_API_URL || "https://efaschoolburewala.onrender.com"}/users/${id}`, { method: 'DELETE' });
            fetchData();
        } catch (err) { console.error(err); }
    };
    ...
    const handleSave = async (e: React.FormEvent) => {
        e.preventDefault();
        const url = formData.id === 0 ? `${process.env.NEXT_PUBLIC_API_URL || "https://efaschoolburewala.onrender.com"}/users` : `${process.env.NEXT_PUBLIC_API_URL || "https://efaschoolburewala.onrender.com"}/users/${formData.id}`;
