...
export default function NewAdmission() {
    ...
    const searchSiblings = async (query: string) => {
        ...
        try {
            const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "https://efaschoolburewala.onrender.com"}/students/search-siblings?query=${encodeURIComponent(query)}`);
            if (res.ok) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "https://efaschoolburewala.onrender.com"}/students`, {
                method: 'POST',
                                                    ...
                                                    {searchResults.map((student: any) => (
                                                        ...
                                                        >
                                                            <div className="flex-shrink-0 me-3">
                                                                {student.image_url ? (
                                                                    <img
                                                                        src={`${process.env.NEXT_PUBLIC_API_URL || "https://efaschoolburewala.onrender.com"}/${student.image_url}`}
